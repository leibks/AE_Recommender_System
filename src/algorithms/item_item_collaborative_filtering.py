...
def build_item_utility_matrix(utility_matrix, df, user_dic, high_price, low_price, consider_economic=False):
    for index in df.index:
        ...
        if consider_economic:
            ...
            economic_factor = 0
        utility_matrix[product_id][user_dic[name]] = rate + economic_factor
...
def build_item_similarity_matrix(similarity_matrix, utility_matrix, user_names, user_dic):
    for product_id in similarity_matrix.keys():
        ...
        for user_name in user_names:
            if utility_matrix[product_id][user_dic[user_name]] != 0:
                similarity_matrix[product_id][user_dic[user_name]] \
                    = utility_matrix[product_id][user_dic[user_name]] - average
# ==================================== Set up matrix ====================================
...
def predict_single_product_utility(utility_matrix, similar_products, user_name, user_dic):
    ...
    for similar in similar_products:
        utility = utility_matrix[similar[0]][user_dic[user_name]]
...
def find_recommended_products_by_ii(user_name, utility_matrix, similarity_matrix, user_dic, num_recommend):
    idx = user_dic[user_name]
    ...
    for product_id in similarity_matrix.keys():
        if utility_matrix[product_id][idx] == 0:
            ...
            utility_matrix[product_id][idx] = \
                predict_single_product_utility(utility_matrix, similar_products, user_name, user_dic)
...
def find_recommended_products_by_ii_lsh(user_name, utility_matrix, similarity_matrix, user_dic, num_recommend):
